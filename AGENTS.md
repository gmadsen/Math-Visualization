...
- **Storage**: permitted `localStorage` keys are `mvnb.progress.v1` (mastery, `MVProgress`), `mvnb.theme` (dark/light), `mvnb.display` (reader widget/quiz hide), and `mvnb.ui.recentOpen` (index.html "Recently updated" collapse state — `'0'` closed, anything else open; default open). Don't add other ad-hoc keys, cookies, or IndexedDB — anything else goes in memory for the session.
