...
- **Index sections**: the canonical topic → subject mapping is `concepts/sections.json`. `validate-concepts` fails if a registered topic is missing from it. The 12 sections:
  ...
  12. **Control theory & optimization** (pink) — linear / convex programming, network flows, dynamic programming, optimal control, stochastic control + mathematical finance. *Empty at section open*; first topics are queued under "Missing-topic candidates" in PLAN.md.
