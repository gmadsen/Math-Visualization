...
scripts/lib/                  shared libs: content-model, audit-utils (incl. parseTopicHtmlSafe), html-walk, ajv, widget-instances, script-scan, json-block-writer, html-injector
...
- **Quality gates** (`validate-concepts`, `validate-katex`, `validate-schema`, `validate-widget-params`, `validate-meta-pages`, `validate-recent-updates`, `audit-concept-latex`, `test-widget-renderers`, `test-widget-hydration`, `test-gesture-engines`, `smoke-test`, `test-roundtrip`, `audit-callbacks`, `audit-starter-concepts`) — non-zero on failure; CI fails.
...
- **Shared libraries** in `scripts/lib/` — [`content-model.mjs`](./scripts/lib/content-model.mjs) (`loadContentModel()` returns memoized `concepts`, `quizBanks`, `byPrereq`, `crossTopicEdges`, `ownerOf`, etc.); [`audit-utils.mjs`](./scripts/lib/audit-utils.mjs) (regex helpers + `parseTopicHtmlSafe()` and `recoverDroppedNodes()` for the parser silent-drop workaround); [`html-walk.mjs`](./scripts/lib/html-walk.mjs) (`matchClose`, `balancedRange`); [`ajv.mjs`](./scripts/lib/ajv.mjs) (`makeAjv()`); [`widget-instances.mjs`](./scripts/lib/widget-instances.mjs) (registry widget instances + fixtures, raw `params` for the widget test suite); plus `script-scan.mjs`, `json-block-writer.mjs`, `html-injector.mjs`. New audits import these rather than re-parsing JSON.
...
- **PLAN.md bullets are deleted at ship, not after** — if your work resolves a bullet in [`PLAN.md`](./PLAN.md), the PR isn't "shipped" until that bullet is removed in the same PR. For already-merged work whose bullet was missed, fix it immediately via a doc-only direct-to-main commit. **Why:** stale bullets mislead every future session's "what next" — the agent reads PLAN.md fresh each conversation and has no other signal that the work is done. This has been a recurring failure (e.g. the section-map parallelization fix shipped in PR #185 but its bullet sat at the top of PLAN.md for weeks). **How to apply:** after merging any PR, re-read PLAN.md end-to-end and grep it for keywords from the PR title/scope; if a bullet describes the shipped work, delete it. `git log` is the audit trail — no "Shipped recently" section.
...
- **Storage**: permitted `localStorage` keys are `mvnb.progress.v1` (mastery, `MVProgress`), `mvnb.theme` (dark/light/high-contrast), `mvnb.display` (reader widget/quiz hide), and `mvnb.ui.recentOpen` (index.html "Recently updated" collapse state — `'0'` closed, anything else open; default open), `mvnb.collapsedSections.v1` (index.html per-section collapse state), `mvnb.search.recent.v1` (search.html recent queries), and `mvnb.tours.v1` (tours.html reading position for the resume banner). Don't add other ad-hoc keys, cookies, or IndexedDB — anything else goes in memory for the session. One permitted **`sessionStorage`** key: `mvnb.tourHandoff` (a one-shot `{tour, stop, page}` payload `tours.html` writes when a stop is clicked and `js/tour-banner.js` reads-then-deletes on the destination, so the tour-guide banner knows which tour you entered).
...
- **Index sections**: the canonical topic → subject mapping is `concepts/sections.json`. `validate-concepts` fails if a registered topic is missing from it. The 13 sections:
  ...
  5. **Probability & statistics** (cyan) — probability theory, mathematical statistics, stochastic processes/calculus, large deviations, random walks, information theory, mathematical biology.
  ...
  10. **Combinatorics & graph theory** (yellow/green) — spectral graphs, matroids, probabilistic method, extremal, simplicial complexes, expanders, Ramsey, additive NT, knot polynomials, coding theory.
  11. **Mathematical physics** (orange/violet) — classical Hamiltonians, statistical mechanics, Schrödinger, gauge theory.
  12. **Control theory & optimization** (pink) — linear / convex programming, network flows, dynamic programming, optimal control, stochastic control + mathematical finance. The decision-making half (MDPs, POMDPs, reinforcement learning, game theory) is queued under the breadth-expansion program in PLAN.md.
  13. **Learning theory & data science** (blue) — statistical learning theory, kernels & RKHS, deep-learning theory, probabilistic graphical models, information geometry, score-based generative models, causal inference. *Empty at section open*; topics queued under the breadth-expansion program in PLAN.md.
...
- **PLAN.md bullet check**: if any item in [`PLAN.md`](./PLAN.md) describes work this PR resolves, delete that bullet in the same PR. After merging, re-read PLAN.md and audit nearby bullets — if "the fix shipped in PR #N" is true for any of them, remove them in a doc-only direct-to-main commit. See the "PLAN.md bullets are deleted at ship" pitfall above for why this matters.
...
- Don't re-implement concept / quiz / topic-HTML parsing in a new audit script. Import `loadContentModel()` from [`scripts/lib/content-model.mjs`](./scripts/lib/content-model.mjs) — it memoizes a normalized model across all topics (concepts, quiz banks, reverse adjacency, cross-topic edges, parsed HTML). Shared audit helpers live in [`scripts/lib/audit-utils.mjs`](./scripts/lib/audit-utils.mjs). Bespoke `JSON.parse` loops in new audits are a code-smell the reviewer will flag. *Exception: scripts that need raw verbatim JSON bytes with role-specific error semantics — the bundle builders (`build-concepts-bundle`, `build-quizzes-bundle`), `audit-bundle-staleness`, `inject-index-stats`, `audit-starter-concepts`, `validate-meta-pages` (validates index.html/meta-page internals the content model doesn't expose), and `validate-recent-updates` (gates generated browser artifacts) — stay bespoke by design; and for widget-block instances in `content/*.json`, import [`scripts/lib/widget-instances.mjs`](./scripts/lib/widget-instances.mjs) instead (the content model never reads `content/*.json`).*
