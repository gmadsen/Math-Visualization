...
- **Storage**: permitted `localStorage` keys are `mvnb.progress.v1` (mastery, `MVProgress`), `mvnb.theme` (dark/light), `mvnb.display` (reader widget/quiz hide), and `mvnb.ui.recentOpen` (index.html "Recently updated" collapse state — `'0'` closed, anything else open; default open), `mvnb.collapsedSections.v1` (index.html per-section collapse state), and `mvnb.search.recent.v1` (search.html recent queries), and `mvnb.tours.v1` (tours.html reading position for the resume banner). Don't add other ad-hoc keys, cookies, or IndexedDB — anything else goes in memory for the session.
