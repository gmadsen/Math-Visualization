...
export function renderScript(params) {
  ...
  return (
    ...
    `  BG.appendChild(SVG('circle',{cx:PX(-MU),cy:PY(0),r:11,fill:'var(--cyan)','fill-opacity':0.85,stroke:'var(--line)'}));\n` +
    `  BG.appendChild(SVG('circle',{cx:PX(1-MU),cy:PY(0),r:5,fill:'var(--mute)',stroke:'var(--line)'}));\n` +
    ...
    `    BG.appendChild(SVG('path',{d:'M'+(PX(p[0])-4)+' '+PY(p[1])+' L'+(PX(p[0])+4)+' '+PY(p[1])+' M'+PX(p[0])+' '+(PY(p[1])-4)+' L'+PX(p[0])+' '+(PY(p[1])+4),stroke:'var(--orange)','stroke-width':1.6}));\n` +
    `    const t=SVG('text',{x:PX(p[0]),y:PY(p[1])+(dy||-8),'font-size':11,fill:'var(--orange)','text-anchor':'middle'}); t.textContent=name; BG.appendChild(t);\n` +
    `  }\n` +
    ...
    `    }\n` +
    `    return { d:d, outcome:outcome, t:(n+1)*DT, end:s };\n` +
    `  }\n` +
    ...
    `    const path=SVG('path',{d:r.d,fill:'none',stroke:'var(--yellow)','stroke-width':1.5,opacity:0.9,'pointer-events':'none'});\n` +
    ...
    `    const dL4=Math.hypot(x-L4[0],y-L4[1]);\n` +
    `    if(dL4<0.25&&sp<0.15&&!r.outcome) msg+=' \\u2014 a tadpole hugging L4: Coriolis turns the potential hilltop into a stable parking spot';\n` +
    `    out.innerHTML=msg;\n` +
    `  }\n` +
