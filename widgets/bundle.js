...
window.__MVWidgets = [
  ...
  {
    "slug": "build-a-formula",
    "family": "build-a-formula",
    "dimension": "2d",
    "gesture": "compose-evaluate",
    "role": "explore",
    "title": "build-a-formula widget params",
    "description": "A self-contained 'compose-and-evaluate' engine for first-order satisfaction over a single binary relation R. The reader builds a sentence by clicking tiles (quantifiers forall/exists over x and y, the connectives and/or/not/implies, and the atoms R(x,y), R(y,x), R(x,x), R(y,y), x=y) which append in prefix order; the engine parses the prefix sentence to an abstract syntax tree and evaluates it on each of several small author-supplied structures (finite directed graphs), showing live which structures satisfy it. This is the symbolic-input modality the corpus otherwise lacks: you assemble a formula and watch the models split into satisfy / not. The renderer owns the tile palette, the prefix builder with backspace/clear, the well-formedness check, the FO evaluator (recursive over the AST and the finite domain), the per-structure satisfaction diagrams, and the readout; the structures are author-supplied. The gesture is 'compose-evaluate' — building a syntactic object and watching it evaluate. Pure DOM/SVG; jsdom-safe (no getScreenCTM/rAF; click-driven).",
    "requiredParams": [
      "widgetId",
      "formulaId",
      "svgId",
    ...
    "readmeExcerpt": "Self-contained **\"compose-and-evaluate\"** engine for first-order satisfaction over one binary relation $R$ — the corpus's `compose-evaluate` gesture (the symbolic-input modality it otherwise lacks: you *assemble* a formula rather than pick one). The reader builds a sentence by clicking **prefix tiles** (quantifiers $\\forall/\\exists$ over $x,y$; the connectives $\\land\\lor\\lnot\\to$; the atoms $R(x,y),R(y,x),R(x,x),R(y,y),x=y$); the engine parses it to an AST and evaluates it on several small structures (finite directed graphs), marking live which ones **model** it. First home: `first-order-logic-and-completeness §semantics`.",
    "hasExample": false,
    "exampleParams": null,
    "exampleMarkup": null,
    "exampleScript": null
  ...
  },
  {
    ...
    "readmeExcerpt": "Self-contained **\"click-to-multiply\"** engine for a graded ring's multiplication table — the corpus's `click-multiply` gesture, built for **cohomology rings** (the cup product). The reader picks a space from a dropdown and **clicks a cell** of the basis × basis grid; the engine shows the product $\\alpha\\smile\\beta$ as an element of the ring, with the **degrees adding** ($H^p\\times H^q\\to H^{p+q}$), the **graded-commutativity sign** $(-1)^{pq}$ relating the cell to its transpose, and products that **vanish** ($x^{n+1}=0$ past the top dimension, $a^2=0$ in an exterior algebra) shown as $0$. It turns \"cohomology is a ring\" from a sentence into a table you can read off. First home: `cohomology-and-duality §cohomology-cup`.",
    "hasExample": false,
    "exampleParams": null,
    "exampleMarkup": null,
    "exampleScript": null
  ...
  },
  {
    ...
    "description": "A self-contained 'click to seed' widget for ergodic theory on the 2-torus. The reader CLICKS a point in the unit square [0,1)^2 (the torus) to set an initial condition, then watches its forward orbit under a measure-preserving map fill the torus — or not. A 'next map' button cycles three canonical maps with different ergodic behaviour: an irrational rotation (ergodic but not mixing — the orbit equidistributes evenly), Arnold's cat map (mixing — a stronger property that scrambles whole regions, though a single orbit still equidistributes), and a rational rotation (periodic — the orbit is a finite set, never equidistributing). A '+ steps' button extends the orbit (capped so the DOM cannot bloat); a coverage readout reports how many cells of a fine grid the orbit's support has covered, so the reader can watch one orbit's support fill the torus (ergodic) or stall on a finite set (periodic) — the geometric shadow of Birkhoff's time-average = space-average, with full coverage a hint rather than a proof. Pure DOM/SVG; jsdom-safe and fully deterministic: the orbit iteration has no randomness and the first render shows the default seed; pointer→torus mapping uses getBoundingClientRect inside the click handler only.",
    "requiredParams": [
      "widgetId",
      "svgId",
      "outputId",
