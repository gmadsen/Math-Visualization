...
window.__MVWidgets = [
  ...
  {
    "slug": "cohomology-stiefel-whitney-rpn",
    "family": "cohomology-stiefel-whitney-rpn",
    "dimension": "2d",
    "gesture": "slider",
    "role": "exploratory",
    "title": "cohomology-stiefel-whitney-rpn widget params",
    "description": "The Stiefel-Whitney classes of real projective space RP^n, computed from the Whitney sum formula as an (n+1)-fold cup product. Because T(RP^n) (+) trivial line = (n+1) copies of the tautological line bundle (each with total class 1+a), the Whitney sum formula gives w(T RP^n) = (1+a)^{n+1} in the mod-2 cohomology ring H*(RP^n; Z/2) = Z/2[a]/(a^{n+1}), so w_i = C(n+1, i) mod 2. A slider sets n; the widget draws Pascal's triangle mod 2 (the Sierpinski pattern) up to row n+1, highlights that row and boxes its first n+1 entries (the surviving classes w_0..w_n, with the a^{n+1} entry crossed out by the truncation), and prints the total class w(T RP^n) explicitly. The readout reads off the consequences: RP^n is orientable iff w_1 = (n+1) mod 2 = 0 iff n is odd, and the total class is trivial (a necessary condition for parallelizability / stable triviality) iff n+1 is a power of 2 -- recovering that RP^1, RP^3, RP^7 are the parallelizable ones. No numeric params beyond the standard widgetId/title/hint.",
    "requiredParams": [
      "widgetId",
      "title"
    ],
    "readmeExcerpt": "Bespoke widget for `cohomology-and-duality.html` (§6 *Characteristic classes preview*, concept `cd-characteristic-preview`). It makes the section's headline application of the cup product — the **Whitney sum formula** — concrete by computing the Stiefel–Whitney classes of $\\mathbb{RP}^n$. Single module, not part of a shared family.",
    "hasExample": false,
    "exampleParams": null,
    "exampleMarkup": null,
    "exampleScript": null
  ...
  {
    "slug": "diff-forms-integration",
    "family": "diff-forms-integration",
    "dimension": "2d",
    "gesture": "click",
    "role": "exploratory",
    "title": "diff-forms-integration widget params",
    "description": "Integrating a 1-form over a 1-chain by pullback to the parameter interval. Pick a 1-form ω = P dx + Q dy and an oriented curve γ; the integral ∫_γ ω = ∫₀¹ [P(γ)x′ + Q(γ)y′] dt is the pullback γ*ω integrated over [0,1], shown as a Riemann sum with the form paired against the tangent γ′(t) at sample points (green where the pairing is positive, pink where negative). Reversing the orientation flips the sign — the ±1 coefficient on the chain — and the angle form ∮ = 2π around a loop enclosing the origin previews de Rham.",
    "requiredParams": [
      "widgetId",
      "title"
    ],
    "readmeExcerpt": "Bespoke module for **differential-forms** §5 (Integration of forms over chains). Shows that integrating a 1-form over an oriented curve is defined by pullback to the parameter interval, and that orientation contributes a sign.",
    "hasExample": false,
    "exampleParams": null,
    "exampleMarkup": null,
    "exampleScript": null
  ...
  {
    "slug": "diff-forms-pullback",
    "family": "diff-forms-pullback",
    "dimension": "2d",
    "gesture": "slider",
    "role": "exploratory",
    "title": "diff-forms-pullback widget params",
    "description": "The pullback of a top form picks up the Jacobian determinant. For a map φ: (u,v) ↦ (x,y) and the area form ω = dx∧dy on the target, the pullback is φ*ω = det(Dφ)·du∧dv. Move the base point and pick a map; the widget shows the unit (du,dv) square in the source plane and its image — the parallelogram spanned by the columns of the Jacobian Dφ — in the target plane, whose signed area is exactly det(Dφ). This is why a change of coordinates picks up the Jacobian exactly once.",
    "requiredParams": [
      "widgetId",
      "title"
    ],
    "readmeExcerpt": "Bespoke module for **differential-forms** §6 (Pullback of forms). Visualizes the single most useful fact about pullback: pulling back a top form picks up the Jacobian determinant.",
    "hasExample": false,
    "exampleParams": null,
    "exampleMarkup": null,
    "exampleScript": null
  ...
  {
    "slug": "fr-decomposition",
    "family": "fr-decomposition",
    "dimension": "2d",
    "gesture": "slider",
    "role": "exploratory",
    "title": "fr-decomposition widget params",
    "description": "The decomposition and inertia groups at a prime P above p in a Galois extension K/Q. Steppers set the ramification index e, residue degree f, and number of primes r (so |G|=n=efr). The widget draws the fixed-field tower Q ⊆ K^D ⊆ K^I ⊆ K with step degrees r (decomposition: p splits into r), f (residue/Frobenius), and e (inertia/ramification), alongside the short exact sequence 1→I→D→Gal(F_{p^f}/F_p)→1 with |D|=ef=|G|/r (orbit–stabiliser), |I|=e, and D/I≅Z/f where Frobenius lives. At an unramified prime e=1, so I is trivial and D is cyclic of order f generated by Frobenius.",
    "requiredParams": [
      "widgetId",
      "title"
    ],
  ...
  {
    "slug": "fr-splitting-types",
    "family": "fr-splitting-types",
    "dimension": "2d",
    "gesture": "click",
    "role": "exploratory",
    "title": "fr-splitting-types widget params",
    "description": "The splitting type of an unramified prime p in a Galois extension K/Q of degree n. Pick n and a divisor pair (r,f) with rf=n; the widget shows the factorization pO_K = P_1···P_r into r primes each of residue degree f, draws Frobenius as r disjoint f-cycles (its cycle shape on the n roots), and reports the factorization of f-bar(x) into r distinct degree-f irreducibles over F_p, the order f of Frobenius, and the name: completely split (r=n,f=1, Frob=identity), inert (r=1,f=n, Frob an n-cycle), or partially split.",
    "requiredParams": [
      "widgetId",
      "title"
    ],
  ...
  },
  {
    ...
    "description": "Why Frobenius traces of a Galois representation see only its semisimplification, shown on a 2-by-2 upper-triangular representation with a REPEATED eigenvalue. The widget displays rho(Frob_p) = [[a, b],[0, a]] with a fixed repeated diagonal value a and a slider for the off-diagonal extension class b. The line spanned by e_1 is always Galois-stable, so rho is REDUCIBLE for every b; for b nonzero it is a non-trivial Jordan block whose ONLY invariant line is span(e_1), so there is no invariant complement and rho is indecomposable and NOT semisimple. It is semisimple only at b = 0, where it is the scalar a*I = its semisimplification rho^ss = diag(a, a). (Using a repeated eigenvalue is what makes the single on-screen matrix genuinely non-diagonalizable for b != 0; with distinct eigenvalues a single matrix is always diagonalizable and non-semisimplicity is only visible at the level of the whole homomorphism.) The trace 2a and determinant a^2 are independent of b, so the non-semisimple rho and its semisimplification have identical Frobenius traces at every prime. The readout draws the consequence: traces are additive on short exact sequences, so tr rho(Frob_p) = tr rho^ss(Frob_p); by Brauer-Nesbitt plus Chebotarev two semisimple l-adic representations are isomorphic iff their Frobenius traces agree on a density-one set of primes. No numeric params beyond the standard widgetId/title/hint.",
    "requiredParams": [
      "widgetId",
      "title"
    ],
  ...
  {
    "slug": "langlands-gl2-modularity",
    "family": "langlands-gl2-modularity",
    "dimension": "2d",
    "gesture": "click",
    "role": "exploratory",
    "title": "langlands-gl2-modularity widget params",
    "description": "The proven GL_2 case of global Langlands over Q, made arithmetic: pick an elliptic curve E/Q of prime conductor N (11a, 19a, 37a), and the widget point-counts a_p(E) = p+1 - #E(F_p) live for each good prime p and shows it equals the q-expansion coefficient a_p(f) of the weight-2 newform f in S_2(Gamma_0(N)) attached to E. The Modularity Theorem (Wiles for semistable, Taylor-Wiles, Breuil-Conrad-Diamond-Taylor in general) is exactly the statement that these two sequences agree for all p, equivalently L(E,s) = L(f,s); on the Galois side it is the assertion that the 2-dimensional l-adic representation rho_{E,l}: Gal(Q-bar/Q) -> GL_2(Q_l) on the Tate module matches the cuspidal automorphic representation pi_f of GL_2(A_Q). The newform coefficients are taken from LMFDB (labels 11.2.a.a, 19.2.a.a, 37.2.a.a) and the curve-side a_p are computed by genuine point-counting in the browser, so the visible match is two independent computations agreeing. The conductor prime p=N (bad multiplicative reduction) is excluded from the point-counting table. No numeric params beyond the standard widgetId/title/hint.",
    "requiredParams": [
      "widgetId",
      "title"
    ],
    "readmeExcerpt": "Bespoke widget for `langlands-program.html` (§6 *Modularity: the proven $\\GL_2$ case*, concept `gl2-modularity`). Single module, not part of a shared family.",
    "hasExample": false,
    "exampleParams": null,
    "exampleMarkup": null,
    "exampleScript": null
  ...
  {
    "slug": "mt-sigma-algebra",
    "family": "mt-sigma-algebra",
    "dimension": "2d",
    "gesture": "click",
    "role": "exploratory",
    "title": "mt-sigma-algebra widget params",
    "description": "The σ-algebra generated by a family of subsets, on the finite set X={1,2,3,4}. Toggle generators from a bank into the family E; the widget computes the partition of X into atoms (points sharing the same membership pattern across all generators), colours X by atom, and lists every element of σ(E) — exactly the unions of atoms, so |σ(E)|=2^(#atoms). It checks the three σ-algebra axioms (∅∈A, closed under complement, closed under union) on the result, and presets sweep from the trivial {∅,X} through a single event and a partition to the discrete 2^X.",
    "requiredParams": [
      "widgetId",
      "title"
    ],
    "readmeExcerpt": "Bespoke module for **measure-theory** §2 (σ-algebras and measures). Builds the σ-algebra generated by a family of subsets and makes the atom description tangible.",
    "hasExample": false,
    "exampleParams": null,
    "exampleMarkup": null,
    "exampleScript": null
