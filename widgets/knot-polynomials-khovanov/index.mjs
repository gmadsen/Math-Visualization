...
export function renderScript(params) {
  ...
  return (
    ...
    `  function polyStr(P){ var ks=Object.keys(P).map(Number).sort(function(a,b){return a-b;}); if(!ks.length) return '0'; return ks.map(function(p,idx){ var c=P[p]; var mag=Math.abs(c); var coef=(mag===1)?'':(''+mag); var pw=(p===0)?(coef||'1'):(coef+'q'+sup(p)); var sign=(idx===0)?(c<0?'\\u2212':''):(c<0?' \\u2212 ':' + '); return sign+pw; }).join(''); }\n` +
    ...
    `    var contentH=gridBot+94; svg.setAttribute('viewBox','0 0 600 '+contentH); svg.setAttribute('height',contentH);\n` +
