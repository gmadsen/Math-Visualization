...
export function renderMarkup(params) {
  ...
  return (
    ...
    `    <label for="${widgetId}-im">Im&#8201;&#964;</label>\n` +
    `    <input type="range" id="${widgetId}-im" min="-2.8" max="2.8" value="1.4" step="0.1">\n` +
...
export function renderScript(params) {
  ...
  return (
    ...
    `    if(!isFinite(re))re=0.5; if(!isFinite(im))im=1.4; if(!isFinite(th))th=0;\n` +
    `    var degenerate=(Math.abs(im)<0.12);\n` +
    ...
    `    // ===== RIGHT: moduli space C\\\\R (both halves; real axis through the middle) =====\n` +
    `    var qx0=310, qx1=545, qy0=70, qy1=292, reMin=-2, reMax=3, imAbs=3.2, qyMid=(qy0+qy1)/2;\n` +
    `    function mscr(t){ return [qx0+(t[0]-reMin)/(reMax-reMin)*(qx1-qx0), qyMid - t[1]/imAbs*((qy1-qy0)/2)]; }\n` +
    `    txt(qx0+2, qy0-2, 'moduli space  {labeled triangles}/sim.  \\u2245  \\u2102 \\u2216 \\u211d', {size:10, fill:'var(--mute)'});\n` +
    `    // Im axis (re=0)\n` +
    `    var zx=mscr([0,0])[0]; svg.appendChild(mk('line',{x1:zx,y1:qy0,x2:zx,y2:qy1,stroke:'var(--line)','stroke-width':1}));\n` +
    `    // excluded real axis (degenerate / collinear) down the middle\n` +
    `    svg.appendChild(mk('line',{x1:qx0,y1:qyMid,x2:qx1,y2:qyMid,stroke:'var(--pink)','stroke-width':2,'stroke-opacity':0.55}));\n` +
    `    txt(qx1, qyMid-4, 'Im \\u03c4 = 0 excluded', {size:8.5, fill:'var(--pink)', anchor:'end'});\n` +
    `    txt(qx0+2, qy0+13, 'Im \\u03c4 > 0: counterclockwise', {size:8, fill:'var(--mute)'});\n` +
    `    txt(qx0+2, qy1-3, 'Im \\u03c4 < 0: clockwise mirror', {size:8, fill:'var(--mute)'});\n` +
    `    // equilateral special point (+ its clockwise mirror)\n` +
