...
export function renderScript(params) {
  ...
  return (
    ...
    `    out.textContent = 'At the base point the differential D\\u03c6 sends the tangent vectors du, dv (the unit square on the left, du\\u2227dv = 1) to D\\u03c6\\u00b7du and D\\u03c6\\u00b7dv \\u2014 the columns of the Jacobian, spanning the parallelogram on the right. Pulling back the area form \\u03c9 = dx\\u2227dy gives \\u03c6*\\u03c9 = det(D\\u03c6)\\u00b7du\\u2227dv = '+det.toFixed(2)+'\\u00b7du\\u2227dv, so that parallelogram has signed area det(D\\u03c6) = '+det.toFixed(2)+'. '+(Math.abs(det-1)<1e-9?'Here det = 1, so \\u03c6 preserves the area form (a shear).':(mi===2?'For this map det = 1 + 0.5u depends on the base point \\u2014 slide u and watch the area change.':'det is constant for this linear map.'))+' This is a pointwise (infinitesimal) statement: for a nonlinear \\u03c6 a finite square would map to a curved region, but the area form still scales by det(D\\u03c6) at each point \\u2014 which is exactly why a change of coordinates picks up the Jacobian once: \\u222b_{\\u03c6(R)} dx\\u2227dy = \\u222b_R det(D\\u03c6)\\u00a0du\\u2227dv.';\n` +
    `  }\n` +
