...
export function renderScript(params) {
  ...
  return (
    ...
    `    var terms=n+1, x0=80, x1=478, y=92, bw=Math.min(70, (x1-x0)/terms - 8);\n` +
    `    function cx(i){ return x0 + (terms<=1? (x1-x0)/2 : i*(x1-x0)/(terms-1)); }   // i=0 is K_n (left), i=terms-1 is K_0 (right)\n` +
    `    txt(cx(0)-bw/2-8, y+4, '0 \\u2192', {anchor:'end', size:11, fill:'var(--mute)'});\n` +
    ...
    `    }\n` +
    `    txt(cx(terms-1)+bw/2+8, y+4, '\\u2192 0', {size:11, fill:'var(--mute)', anchor:'start'});\n` +
    `    // facts\n` +
    ...
    `    lines.push('Theorem: a\\u2081,\\u2026,a_' + n + ' is a REGULAR SEQUENCE (each a_i a non-zero-divisor on R/(a\\u2081,\\u2026,a_{i\\u22121})) \\u21d2 K\\u2022 is exact in positive degrees, H\\u2080 = R/(a\\u2081,\\u2026,a_' + n + '). The converse holds when R is Noetherian local (or graded with the a_i in the irrelevant ideal) \\u2014 the standard setting here \\u2014 giving the clean \\u21d4. Then K\\u2022 is a finite free RESOLUTION of that quotient \\u2014 used to compute Tor and Ext against it, and to read off depth (the length of a maximal regular sequence in an ideal).');\n` +
