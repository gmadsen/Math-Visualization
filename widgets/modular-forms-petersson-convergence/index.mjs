...
export function renderMarkup(params) {
  ...
  return (
    ...
    `    <input type="range" id="${widgetId}-Y" min="100" max="500" value="180" step="2">\n` +
...
export function renderScript(params) {
  ...
  return (
    ...
    `  var PX0=66, PX1=452, PTOP=50, PBOT=246, LMIN=-21, LMAX=7;\n` +
    ...
    `    out.textContent='The Petersson product is \\u27e8f,g\\u27e9 = \\u222b_{\\u0393\\\\\\u210d} f \\u00b7 conj(g) \\u00b7 y^{k\\u22122} dx dy on weight-k forms. The hyperbolic measure y\\u207b\\u00b2 dx dy and the combination |f|\\u00b2 y^k are each SL\\u2082(\\u2124)-invariant (under \\u03b3 the factor |c\\u03c4+d|\\u207b\\u00b2\\u1d4f in |f|\\u00b2 is exactly cancelled by y \\u21a6 y/|c\\u03c4+d|\\u00b2), so the integrand descends to the fundamental domain and the pairing is well-defined. All the convergence drama is at the cusp i\\u221e: by Parseval the x-average of |f|\\u00b2 over the strip |x|\\u2264\\u00bd is \\u03a3_{n\\u22650} |a_n|\\u00b2 e\\u207b\\u2074\\u207f\\u02b8 (with q = e^{2\\u03c0i\\u03c4}), so the cusp-neighborhood part of \\u2016f\\u2016\\u00b2 is \\u222b H(y) dy with H(y) = (\\u03a3 |a_n|\\u00b2 e\\u207b\\u2074\\u207f\\u02b8) y^{k\\u22122}, here k=12. For the CUSP FORM \\u0394 the constant term a\\u2080 = 0, so H(y) \\u2248 e\\u207b\\u2074\\u02b8 y\\u00b9\\u2070 \\u2014 the exponential crushes the polynomial and \\u222b\\u0394 converges (it has essentially reached '+IDfull.toExponential(2)+' by Y='+YMAX+'; right now \\u222b_{\\u221a3/2}^{'+Y.toFixed(2)+'}\\u0394 = '+ID.toExponential(2)+'). For the EISENSTEIN series E\\u2081\\u2082 the constant term a\\u2080 = 1, so H(y) \\u2248 y\\u00b9\\u2070 \\u2192 \\u221e and \\u222bE\\u2081\\u2082 = '+IE.toExponential(2)+' just keeps climbing \\u2014 it DIVERGES. That is precisely why the Petersson product lives on the space S_k of cusp forms (vanishing at every cusp), where it is a genuine Hermitian inner product; the Hecke operators T_p are self-adjoint for it, which forces a basis of simultaneous eigenforms with real eigenvalues.';\n` +
    `  }\n` +
