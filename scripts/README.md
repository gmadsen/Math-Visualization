...
| [`test-gesture-engines.mjs`](./test-gesture-engines.mjs) | Interaction tests for the self-contained shared gesture engines (draggable-points-2d, graph-edit-2d, vector-field-flow-2d, sketch-curve-2d, xy-parameter-pad, animated-svg-2d), which have no `js/widget-*.js` library and so are skipped by the hydration test. Boots every `content/*.json` instance in jsdom with the canonical page helpers + an identity getScreenCTM/createSVGPoint polyfill, drives the gesture with synthetic pointer events (drag a handle, click-seed, add a vertex, sketch a stroke, drag the puck, scrub the timeline), and asserts an observable state change with no script errors. |
