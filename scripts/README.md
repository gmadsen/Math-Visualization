...
| [`audit-math-rendering-leaks.mjs`](./audit-math-rendering-leaks.mjs) | Math-rendering hazards the structural validators miss: CLASS A — raw `<letter` inside `$…$` (HTML tag-open → swallows prose, reader-visible content loss); CLASS B — `renderMathInElement()` lacking a single-`$` delimiter (silent no-op, advisory); CLASS C — LaTeX inside SVG `<text>` (can't render, advisory). `--strict` exits 1 on any CLASS A hit (the gate form); `--write` dumps `audits/math-rendering-leaks.md`. |
