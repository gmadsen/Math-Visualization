...
| [`lib/widget-instances.mjs`](./lib/widget-instances.mjs) | `listRegisteredSlugs()`, `walkWidgetBlocks()`, `contentInstancesBySlug()`, `loadInstances(slug)`, `loadInstancesPerSlug()` — every `{type:"widget", slug, params}` block across `content/*.json` plus the `widgets/<slug>/example.json` / `examples/*.json` test fixtures. Returns raw `params` (deliberately *not* the normalized content model) for the widget test suite (`test-widget-renderers`, `test-widget-hydration`, `test-gesture-engines`). |
