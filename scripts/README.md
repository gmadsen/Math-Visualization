...
| [`migrate-inline-widget.mjs`](./migrate-inline-widget.mjs) | Lift one inline `widget` block + its paired `widget-script` block into a bespoke artifact-style registry slug. Auto-handles 4 dialects: `<span>` vs `<div>` ttl/hint, multi-line `.hd` headers, legacy `{type:"widget", id, html, script}` shape, leading whitespace in legacy scripts. Generates `widgets/<slug>/{schema.json, index.mjs, README.md}` plus the content-JSON rewrite. |
