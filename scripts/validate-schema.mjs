...
// concepts/*.json — skip registry files (index, capstones, sections, tags) and
// the built bundle. These have their own shapes; only topic concept files get
// validated against schemas/concept.schema.json.
const conceptFiles = listJsonFiles(conceptsDir, {
  skip: ['index.json', 'capstones.json', 'sections.json', 'tags.json'],
});
let conceptOk = 0;
