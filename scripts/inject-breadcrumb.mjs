...
//
// The map is written ONCE to js/section-map.js (a single shared file
// containing `window.__MV_SECTION_MAP = { … };`) and each topic page
// references it via a tiny script-loader tag (in `doc.rawHead`):
//
...
//
// This shared-file arrangement is parallelization-friendly: adding or
// renaming a topic only mutates js/section-map.js + the affected topic +
// index.html, instead of rewriting the inlined map in every topic page
// (the prior layout caused every topic-add PR to touch all 190 topic
// HTMLs and content/*.json).
//
...
//
// Modes:
//   default      Audit only. Prints a per-topic list of out-of-sync JSONs and
//                flags js/section-map.js drift if present. Exits 0 on per-topic
//                drift (advisory; roundtrip catches it), but exits 1 on
//                js/section-map.js drift — that file has no other CI safety net,
//                so a stale shared map would silently break every topic page.
//   --fix        Write js/section-map.js from the freshly-parsed map; strip
//                any existing fenced blocks from the JSON's rawHead /
...
// js/section-map.js — single shared file containing the same map every page
// previously inlined. Generated content is byte-stable thanks to the sorted
// stableStringify above, so re-runs are no-ops unless index.html changed.
const sectionMapJsPath = join(repoRoot, 'js', 'section-map.js');
const sectionMapJsContent =
  '// Auto-generated by scripts/inject-breadcrumb.mjs. DO NOT EDIT.\n' +
  '// Source of truth is the section / card layout in index.html.\n' +
  '// Loaded once per topic page (replaces the prior per-page inline map\n' +
  '// so topic-add PRs do not rewrite every other topic\'s <head>).\n' +
  'window.__MV_SECTION_MAP = ' + sectionMapJson + ';\n';
...
let skippedNoProg = 0;
let skippedNoNav = 0;
let sectionMapJsTouched = false;
let sectionMapJsDrift = false;

// 5a. Reconcile js/section-map.js (the shared loader file).
const existingSectionMapJs = existsSync(sectionMapJsPath)
  ? readFileSync(sectionMapJsPath, 'utf8')
  : null;
if (existingSectionMapJs !== sectionMapJsContent) {
  if (FIX) {
    writeFileSync(sectionMapJsPath, sectionMapJsContent);
    sectionMapJsTouched = true;
  } else {
    sectionMapJsDrift = true;
    outOfSync.push('js/section-map.js (missing or stale)');
  }
}
...
if (FIX) {
  ...
  if (sectionMapJsTouched) console.log('  js/section-map.js rewritten');
...
// Audit mode. Per-topic drift is advisory (roundtrip catches it strictly), but
// js/section-map.js drift hard-fails CI — nothing else in the chain compares
// the shared file's bytes, so a stale or missing map would silently ship a
// broken breadcrumb to every page.
if (outOfSync.length === 0) {
  console.log('OK: every topic JSON carries an up-to-date breadcrumb injection.');
  process.exit(0);
}
console.log('');
console.log(`OUT OF SYNC (${outOfSync.length} item(s)):`);
for (const line of outOfSync) console.log(`  - ${line}`);
console.log('');
console.log('Re-run with --fix to apply.');
if (sectionMapJsDrift) {
  console.error('FAIL: js/section-map.js is missing or stale. This breaks the breadcrumb on every topic page.');
  process.exit(1);
}
process.exit(0);
