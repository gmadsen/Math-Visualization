...
const STEPS = [
  ...
  // Canonical cross-page nav row on every secondary meta page (--fix
  // normalizes + rewrites the row by maximal-munch from the backlink; audit
  // mode gates drift/duplication). Writes meta-page HTML directly — these
  // aren't topic pages, so they're outside the content round-trip.
  ...
  // site (the #518 near-miss). This MUST run BEFORE build-recent-updates:
  // the builder unconditionally regenerates both files (fix:false only gates
  // --fix appending, which it ignores), so a gate placed after it would only
  // ever see the fresh regenerated bytes and never the broken committed ones
