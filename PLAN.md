...
2. **Hamiltonians-figure family promotion.** The 6 hamiltonians widgets (`hamiltonians-canonical-transform`, `-conserved-quantity`, `-flow-portrait`, `-kam-tori`, `-kepler-orbits`, `-phase-space-cell`) share 90 %+ of their chrome (svg, output panel, ariaLabel, sectionComment). Collapse into a shared `hamiltonians-figure` slug with a `oneOf` over the input controls (`pick` / `picks` / `slider` / `sliders` / `buttons` / `buttonsTrailing`), keeping `bodyScript` as the only artifact field. Per `widgets/README.md` § "Structured vs. artifact-style adoption", this is the one open promotion candidate — the other ~150 artifact-style slugs are intentionally one-off and stay as-is.
