...
- 1648 widgets, 100% registry-driven. Interactive baseline gated via `audits/static-widgets-baseline.json`
...
- **Next gesture engines worth building:** drag-the-roots polynomial (FTA / Vieta — drag complex roots, watch the curve/coefficients); point-cloud → convex hull / Delaunay / Voronoi; Bézier de Casteljau. **Verify no existing widget already covers the concept** — the corpus is mature (ε–δ is already a clickable on `real-analysis §real-continuity`, conformal-grid is a static illustration). Six new gesture engines now ship (play / draw / graph-edit / click-seed / drag-basis / drag-probe). All jsdom-safe (no rAF/getScreenCTM at init); pattern per `widgets/animated-svg-2d` / `widgets/sketch-curve-2d` / `widgets/graph-edit-2d` / `widgets/vector-field-flow-2d` / `widgets/linear-transform-2d` / `widgets/complex-map-2d`.
