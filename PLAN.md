...
- 149 concepts lack a widget in their owning section (was 177; nine new topics in PRs #131–#140 each shipped 5–7 widgets; RMT and GIT in PRs #155–#156 each shipped 7)
- 1233 widgets, 100% registry-driven (PR #70 zero-baseline sweep — `audit-no-inline-widgets` now hard-fails on any non-registry widget). 1229 interactive, **4 static across 4 pages** — the widget-fix wave PRs #142–#150 retrofitted 56 widgets across 11 dense pages, PR #152 added `MV*.init` recognition to the audit (50→6 statics in one shot), PR #154 added dynamic-id-construction detection (6→4). The 4 residuals are genuinely-static SVG/table illustrations (class-field-theory#w-dict, cocartesian-fibrations#w-universal-leftfib, galois L643, grothendieck-topologies-sites#w-giraud) baselined as legitimate.
...
- **Math physics has no `hard` tier and Combinatorics has no `hard` tier.** 770 concepts lack a hard tier corpus-wide; 13 topics in those two sections × ~6 concepts × 2-3 questions each ≈ ~150 of the missing questions. Per "Out of scope", de-prioritized — listed here so the gap is visible, not actioned.
...
A second comparative pass against four more graduate catalogs (Princeton, MIT, Berkeley, Caltech) surfaced these gaps. Most Harvard candidates (cluster algebras, mapping class groups, Khovanov, complex multiplication, Shimura varieties, tropical geometry, surgery, arithmetic statistics) shipped in PRs #131–#140; **Random matrix theory** shipped in PR #155 and **GIT** in PR #156. The residual list below is the next-priority batch.

**Tier 2 (broad multi-university coverage)**:

1. **Heegaard Floer / knot Floer homology** — Princeton MAT568, multiple universities. Bridges `knot-polynomials` (now extended via Khovanov) to `symplectic-manifolds` and 3-manifold topology. Companion to surgery theory.
2. **Coding theory & error-correcting codes** — Caltech Ma 127. Hamming, Reed–Solomon, BCH, low-density parity-check; lattice codes. Bridges `information-theory`, `additive-number-theory`, `mathematics-and-cryptography`. Highly visualizable.
3. **Conformal & CR geometry** — Princeton MAT558/559. Bridges `complex-analysis`, `several-complex-variables` (just shipped), `differential-geometry`. Q-curvature and conformally covariant operators.
...
- **Recently-updated: full-history page.** `index.html` `#recently-updated` shows the last N entries from `recent-updates.json` (generated by `scripts/build-recent-updates.mjs` from `git log -1 -- quizzes/<slug>.json`). **(1) Collapsible — shipped in PR #157** (toggle persists via `localStorage.mvnb.ui.recentOpen`; default open; cross-tab sync). **(2) Still open:** wire a "full history →" link to a new `updates.html` page that renders the *complete* `recent-updates.json` entries (today the JSON only keeps the most-recent slice for the homepage; the generator may need extending to retain the full timeline, or the page can read from `git log --follow` at build time via a sibling script). Date grouping (month headers) and per-section filtering would help once the list grows past a screen.
...
- **Static-widget retrofit, residual long-tail.** PR #127 promoted `audit-widget-interactivity.mjs` to a CI gate via the baseline file (`audits/static-widgets-baseline.json`) — strict mode fails only if a per-page static count grows. The second wave (PRs #142–#150) retrofitted the densest static pages — each PR dropped its target page to 0 static. Three defect classes surfaced: (A) `bodyScript: ""` with markup but no event wiring; (B) authored `bodyScript` orphaned by a missing `{type:"widget-script", ref:"<widgetId>"}` block; (hybrid) `bodyScript` exists but only delegates to a shared-library `MV*.init(...)` call. The (hybrid) case was resolved by PR #152 (recognize `MV[A-Z][A-Za-z]+\.init\s*\(` as event-binding), dropping static count 50 → 6. A subsequent audit patch added detection for the **dynamic-id construction** pattern (every registry widget's renderScript emits `widgetId: '<id>'` and uses `CFG.widgetId` for DOM lookups; the literal id never appears in `'#<id>'` selector form), which caught the two remaining truly-interactive cases (snake-shape, julia-cascade) and dropped static count 6 → 4. **Now remaining: 4 widgets**, all genuinely-static illustrations with empty `bodyScript` (verbatim slugs): `class-field-theory#w-dict` (reciprocity table), `cocartesian-fibrations#w-universal-leftfib` (pullback square), `galois.html` L643 (three classical impossibilities), `grothendieck-topologies-sites#w-giraud` (Giraud axioms summary). These are LEGITIMATELY static — the SVG / table IS the content — and stay in the baseline as the corpus's permanent "this is not an interactivity bug" anchor.
