...
- 1647 widgets, 100% registry-driven. Interactive baseline gated via `audits/static-widgets-baseline.json`
...
- **Next gesture engines worth building:** a complex-map domain-warp (`w=f(z)`: drag z / animate a grid under a conformal map — high value for complex analysis, very visual, currently under-interactive); an ε–δ drag (drag ε, see the δ band — limits/continuity foundations). Five new gesture engines now ship (play / draw / graph-edit / click-seed / drag-basis). All jsdom-safe (no rAF/getScreenCTM at init); pattern per `widgets/animated-svg-2d` / `widgets/sketch-curve-2d` / `widgets/graph-edit-2d` / `widgets/vector-field-flow-2d` / `widgets/linear-transform-2d`.
