...
- 146 topics, 1038 concepts, 32 capstones
...
- Quiz tiers: v1 = 3072, hard = 1223, expert = 13 (intentionally bottom-of-list — see "Out of scope")
...
- **Math physics has no `hard` tier and Combinatorics has no `hard` tier.** 600 concepts lack a hard tier corpus-wide; 13 topics in those two sections × ~6 concepts × 2-3 questions each ≈ ~150 of the missing questions. Per "Out of scope", de-prioritized — listed here so the gap is visible, not actioned.

## Bigger missing-topic candidates (from the comparative audit)

Listed for the long view; each is a multi-session lift.

- **Algebra side:** `model-categories` (bridge to higher-categories).
...
- **Hard-tier quiz authoring** (600 concepts lack hard tier).
