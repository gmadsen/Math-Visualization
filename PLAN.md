...
- **Static-widget retrofit, third wave (long-tail).** PR #127 promoted `audit-widget-interactivity.mjs` to a CI gate via the baseline file (`audits/static-widgets-baseline.json`) — strict mode fails only if a per-page static count grows. The second wave (PRs #142–#150) retrofitted the densest static pages — each PR dropped its target page to 0 static. Defect classes surfaced during that wave: (A) `bodyScript: ""` with markup but no event wiring; (B) authored `bodyScript` orphaned by a missing `{type:"widget-script", ref:"<widgetId>"}` block (the more common one); (hybrid) `bodyScript` exists but only delegates to `MVProofScrubber.init`/`MVInlineCodeCell.init` — audit regex needs an event verb in the same script. PR #149's drive-by also patched `js/widget-proof-scrubber.js` to forward KaTeX macros to `renderMathInElement`. **Now remaining: 50 static widgets across 29 pages (4-and-3-count long-tail).** Worth a mechanical third-wave pass, plus a manual review of the 1-2 static cases where the SVG is genuinely illustrative and shouldn't be activated.
