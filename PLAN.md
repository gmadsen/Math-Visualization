...
- **Math physics has no `hard` tier and Combinatorics has no `hard` tier.** 519 concepts lack a hard tier corpus-wide; 12 topics in those two sections × ~6 concepts × 2-3 questions each ≈ ~150 of the missing questions. Per "Out of scope", de-prioritized — listed here so the gap is visible, not actioned.
...
- **Hard-tier quiz authoring** (519 concepts lack hard tier).
