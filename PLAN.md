...
- **Hoist semantic params out of verbatim slugs.** Roughly 66 per-widget verbatim slugs share `widgets/_shared/verbatim-renderer.mjs` with opaque `bodyMarkup`/`bodyScript` strings. Migrating them to bespoke renderers with semantic params (slider ranges, color tokens, etc.) lets AJV validation and alt frontends actually inspect each widget. **Status (2026-05-27): the structured-migration program is at its natural end — started at 392, every verbatim widget with a *structurable* shape has been migrated onto the shared `slider-svg-2d` (slider/button/span/select/numinput) and `clickable-diagram` (readout-only / proof-stepper / svg-diagram / table-diagram) renderers via `scripts/migrate-to-{slider-svg-2d,button-stepper,clickable-diagram}.mjs` (strict byte-identity / provably-invisible guards).** The remaining ~66 are uniformly multi-blocker with irregular bodies (readout-before-row, styled rows, leading prose, multi-row, multi-element legends, empty/missing scripts) where structuring = bespoke per-widget effort with no shared-renderer leverage — they are correctly left verbatim. The only remaining levers are new-renderer-shaped or low-priority: the button hard-tail (~28, multi-blocker, low value), the computational `gb-*` Gröbner widgets (text output → would need `inline-code-cell`), and the empty-script cluster (ambiguous script location, needs R&D). **Don't chase a `--normalize`/ordered-block mode for the irregular tail — analysis confirmed it unlocks ~0-2 per attempt (every candidate stacks 2-3 blockers), the same dead-end the button-stepper extensions hit.** See the `project_verbatim_semantic_migration` memory for the exhaustive per-cluster breakdown.
