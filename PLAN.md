...
- 170 topics, 1182 concepts, 34 capstones
...
- Quiz tiers: v1 = 3504, hard = 1223, expert = 13 (intentionally bottom-of-list — see "Out of scope")
...
- **Math physics has no `hard` tier and Combinatorics has no `hard` tier.** 744 concepts lack a hard tier corpus-wide; 13 topics in those two sections × ~6 concepts × 2-3 questions each ≈ ~150 of the missing questions. Per "Out of scope", de-prioritized — listed here so the gap is visible, not actioned.
...
- **Hard-tier quiz authoring** (744 concepts lack hard tier).
