...
- 147 concepts lack a widget in their owning section (was 177; nine new topics in PRs #131–#140 each shipped 5–7 widgets)
- 1219 widgets, 100% registry-driven (PR #70 zero-baseline sweep — `audit-no-inline-widgets` now hard-fails on any non-registry widget). 1113 interactive, 106 static across 40 pages; the widget-fix wave (#142–#150, currently open) retrofits 56 of those 106 (drops to ≈50 static once merged).
...
- **Static-widget retrofit, second wave.** PR #127 promoted `audit-widget-interactivity.mjs` to a CI gate via the baseline file (`audits/static-widgets-baseline.json`) — strict mode fails only if a per-page static count grows. The widget-fix wave #142–#150 retrofits the densest static pages (arith-stats, iwasawa, condensed, mmp, positivity, toric, cohomology-and-duality, deformation-theory, abelian-varieties, infinity-categories, infinity-topoi); each PR drops its page to 0 static. Three defect classes surfaced: (A) `bodyScript: ""` with markup but no event wiring; (B) authored `bodyScript` orphaned by a missing `{type:"widget-script", ref:"<widgetId>"}` block (the more common one); (hybrid) `bodyScript` exists but only delegates to `MVProofScrubber.init`/`MVInlineCodeCell.init` — audit regex needs an event verb in the same script. PR #149's drive-by also patched `js/widget-proof-scrubber.js` to forward KaTeX macros to `renderMathInElement`. After this wave the remaining static long-tail is the 4-and-3-count pages (~50 widgets) — worth doing in another mechanical pass, plus a manual review of the 1-2 static cases where the SVG is genuinely illustrative.
