...
- **Static-widget retrofit, residual long-tail.** PR #127 promoted `audit-widget-interactivity.mjs` to a CI gate via the baseline file (`audits/static-widgets-baseline.json`) — strict mode fails only if a per-page static count grows. The second wave (PRs #142–#150) retrofitted the densest static pages — each PR dropped its target page to 0 static. Three defect classes surfaced during that wave: (A) `bodyScript: ""` with markup but no event wiring; (B) authored `bodyScript` orphaned by a missing `{type:"widget-script", ref:"<widgetId>"}` block (the more common one); (hybrid) `bodyScript` exists but only delegates to a shared-library `MV*.init(...)` call. The (hybrid) case was resolved by patching `audit-widget-interactivity.mjs` to recognize `MV[A-Z][A-Za-z]+\.init\s*\(` as event-binding (the helpers in `js/widget-*.js` do the actual wiring) — this dropped the static count from 50 across 29 pages to **6 across 6 pages** in one shot. PR #149's drive-by also patched `js/widget-proof-scrubber.js` to forward KaTeX macros to `renderMathInElement`. **Now remaining: 6 widgets** — `class-field-theory#w-dict`, `cocartesian-fibrations#w-universal-leftfib`, `dynamical-systems#w-julia-cascade`, `galois.html L643`, `grothendieck-topologies-sites#w-giraud`, `homological#w-snake-shape`. Two patterns left: (a) genuinely-static illustrations (Julia set, Giraud axioms listing) where the SVG is the point; (b) dynamic-id construction (snake-shape uses `document.getElementById(CFG.widgetId + '-svg')`) where the literal id never appears in the script — a separate audit gap. Worth a one-shot pass to either activate the dynamic-id cases or annotate the illustrations.
