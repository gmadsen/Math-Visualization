...
- 149 concepts lack a widget in their owning section (was 177; nine new topics in PRs #131–#140 each shipped 5–7 widgets)
- 1219 widgets, 100% registry-driven (PR #70 zero-baseline sweep — `audit-no-inline-widgets` now hard-fails on any non-registry widget). 1169 interactive, 50 static across 29 pages — the widget-fix wave PRs #142–#150 retrofitted 56 widgets across 11 pages (arith-stats, iwasawa, condensed, mmp, positivity, toric, cohomology-and-duality, deformation-theory, abelian-varieties, infinity-categories, infinity-topoi). Remaining 50 static are 4-and-3-count pages (the long-tail).
- Quiz tiers: v1 = 3561, hard = 1223, expert = 13 (intentionally bottom-of-list — see "Out of scope")
...
- **Math physics has no `hard` tier and Combinatorics has no `hard` tier.** 763 concepts lack a hard tier corpus-wide; 13 topics in those two sections × ~6 concepts × 2-3 questions each ≈ ~150 of the missing questions. Per "Out of scope", de-prioritized — listed here so the gap is visible, not actioned.
...
- **Recently-updated: collapsible + full-history page.** `index.html` `#recently-updated` shows the last N entries from `recent-updates.json` (generated by `scripts/build-recent-updates.mjs` from `git log -1 -- quizzes/<slug>.json`). Two enhancements: (1) make the block collapsible — a returning visitor who's seen the most recent rows shouldn't lose homepage space to it; persist the open/closed state in `localStorage` under a new key (e.g. `mvnb.ui.recentOpen`, alongside `mvnb.progress.v1` / `mvnb.theme` / `mvnb.display` per the storage allowlist in CLAUDE.md). (2) wire a "full history →" link to a new `updates.html` page that renders the *complete* `recent-updates.json` entries (today the JSON only keeps the most-recent slice for the homepage; the generator may need extending to retain the full timeline, or the page can read from `git log --follow` at build time via a sibling script). Date grouping (month headers) and per-section filtering would help once the list grows past a screen.
...
- **Hard-tier quiz authoring** (763 concepts lack hard tier).
