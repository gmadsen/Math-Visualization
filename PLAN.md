...
- 1651 widgets, 100% registry-driven. Interactive baseline gated via `audits/static-widgets-baseline.json`
...
- **Next gesture engines worth building (brainstorm 2026-06-06):** drag-the-roots polynomial (FTA/Vieta); epicycle Fourier (rotating circles trace a curve); osculating circle (drag along a curve, watch κ); bifurcation dial; Lagrange multipliers (level sets kiss the constraint); Newton basins; point-cloud→hull/Delaunay/Voronoi; Bézier de Casteljau. **Verify no existing widget already covers the concept**, and **grep the target page for widgetId collisions before deploying** (e.g. `w-contour` was already taken on complex-analysis). Nine new gesture engines now ship (play / draw / graph-edit / click-seed / drag-basis / drag-probe / drag-on-curve / drag-direction / drag-contour). All jsdom-safe (no rAF/getScreenCTM at init).
