...
- **Color-only prose in widget readouts** (a11y). `audit-accessibility.mjs` flags ~25 "color-only" references ("the orange dot is X"); fix by adding a non-color descriptor ("the dot at the upper-right"). Each batch can chip away at 5-10 spots; per-spot judgment, no automation. Drift risk: new topics ship faster than fixes land.
