...
Start from [`pathway.html`](./pathway.html) to explore prerequisite graphs for capstone goals — pick any of the 34 capstones and the DAG layers every prereq concept by depth. For free-form exploration over the entire 1446-concept graph (no goal required), [`mindmap.html`](./mindmap.html) renders a section-clustered force-directed layout: click a node to focus its k-hop neighborhood, filter by section/level/mastery, and a structural-stats panel surfaces uneven prereq density across the thirteen sections. Both views read the same concept graph; pathway is goal-targeted, mindmap is free-explore.
...
- [Mathematical chaos](./mathematical-chaos.html) — Lyapunov exponents (Oseledec), Feigenbaum universality of period-doubling, strange attractors + Smale's horseshoe, symbolic dynamics + topological entropy, ergodic theory + SRB measures, KAM + Chirikov standard map, Sharkovsky's order
- [Optimal transport](./optimal-transport.html) — draft — fill in once the page has real content
- [Ergodic theory](./ergodic-theory.html) — draft — fill in once the page has real content
...
- [Shimura varieties](./shimura-varieties.html) — draft — fill in once the page has real content
...
- [Graph theory fundamentals](./graph-theory-fundamentals.html) — the foundational layer: degrees and the handshake lemma, trees and Cayley's formula, Eulerian/Hamiltonian paths, planarity and Euler's formula, chromatic number (greedy/Brooks/5-color), and matchings via Hall and König.
- [Ramsey theory](./ramsey-theory.html) — "complete disorder is impossible": pigeonhole and R(3,3)=6, Ramsey numbers and the recurrence/exponential bounds, infinite Ramsey, and the Van der Waerden / Hales–Jewett theorems on monochromatic arithmetic progressions and combinatorial lines.
- [Order theory and lattices](./order-theory-and-lattices.html) — posets and Hasse diagrams, lattices with meet/join, Dilworth's and Sperner's theorems on chains and antichains, Möbius functions and inversion (generalizing inclusion–exclusion), and Knaster–Tarski fixed points on complete lattices.
- [Algebraic combinatorics](./algebraic-combinatorics.html) — integer partitions and Young tableaux, the hook length formula, symmetric functions (monomial/elementary/complete/power-sum) and the Schur basis, the RSK correspondence, and the partition-indexed irreducibles of Sₙ and GLₙ.
- [Polytopes and Ehrhart theory](./polytopes-and-ehrhart.html) — convex polytopes (V/H-representations, the face lattice, the Euler relation), Pick's theorem, the Ehrhart polynomial counting lattice points in dilates, Ehrhart–Macdonald reciprocity, and the h\*-vector / Ehrhart series.
...
- [Game theory](./game-theory.html) — normal-form games and dominance, Nash equilibrium (existence via Kakutani), the von Neumann minimax theorem and its equivalence to LP duality, mixed strategies and the indifference principle, cooperative games (core, Shapley value), and mechanism design (second-price/Vickrey auctions).
- [Reinforcement learning](./reinforcement-learning.html) — learning to act when the MDP's dynamics and rewards are unknown: Monte Carlo and temporal-difference prediction, the TD error, Q-learning and SARSA, function approximation and the deadly triad, and the policy-gradient theorem (REINFORCE, actor–critic).
- [POMDPs and belief states](./pomdps-and-belief-states.html) — partially-observable MDPs: the belief state and Bayes filtering, the belief MDP, the piecewise-linear convex value function and its α-vectors, exact value iteration with pruning, and point-based methods (PBVI/SARSOP), via the Tiger problem.
...
- [Kernel methods and RKHS](./kernel-methods-and-rkhs.html) — positive-definite kernels and the kernel trick, reproducing kernel Hilbert spaces (reproducing property, Moore–Aronszajn), the representer theorem, support vector machines, Gaussian-process regression, and Mercer's theorem with random Fourier features.
...
- [Causal inference](./causal-inference.html) — structural causal models and the do-operator, confounding and the backdoor/front-door criteria, Pearl's do-calculus and identifiability, counterfactuals (abduction–action–prediction), and estimating average treatment effects (adjustment, IPW) under ignorability.
